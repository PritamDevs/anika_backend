...
exports.getReports = async (req, res) => {
 ...
 if (year && month && day) {
  ...
  endDate = new Date(y + 1, 2, 31, 23, 59, 59);
}
    ...
    let returnFilter = { type: "return" };
    
    if (startDate && endDate) {
      returnFilter.date = { $gte: startDate, $lte: endDate };
    }
    
    const returns = await Payment.find(returnFilter);
    ...
    // ================= PAYMENTS =================//
      ...
      let paymentFilter = { type: "payment" };
      if (startDate && endDate) {
        paymentFilter.date = { $gte: startDate, $lte: endDate };
      }
    ...
    const totalSalesWithoutReturn = totalSales - totalReturns;
    
    const pAndL = totalSales - (totalPurchase + totalReturns);
    /* ================= FORMAT RESPONSE ================= */
  ...
  : {
    ...
    res.json({
      summary: {
        ...
        totalPaid, 
        pAndL,
        totalAmount: totalSalesWithoutReturn
      },
      invoices: formattedInvoices
    });
...
exports.getMonthlyChart = async (req, res) => {
  try {
    ...
    invoices.forEach(inv => {
      ...
      const accountingIndex = (monthIndex - 3 + 12) % 12;

      monthlyData[accountingIndex].sales += inv.totalAmount || 0;
      monthlyData[accountingIndex].payments += inv.paidAmount || 0;
